...
// [ ESAME 2021-06-30
	/// @ingroup esame
	/// @brief true se il processo possiede una msg-area
	bool maowner;
//   ESAME 2021-06-30 ]
...
/// indirizzo base della zona dedicata alla msg-area
const vaddr ini_ma_p = ini_utn_p;
/// indirizzo limite della zona dedicata alla msg-area
const vaddr fin_ma_p = ini_ma_p + DIM_PAGINA * MSG_AREA_PAGES;
//   ESAME 2021-06-30 )
/// @}
...
// [ ESAME 2021-06-30
		/// @ingroup esame
		/// @note Inizialmente i processi non possiedono una msg-area.
		p->maowner = false;
//   ESAME 2021-06-30 ]
...
// [ ESAME 2021-06-30
	/// @ingroup esame
	/// @note Se il processo possiede una msg-area, dobbiamo deallocarla
	if (p->maowner) {
...
/// Parte C++ della primitiva macreate()
...
extern "C" void c_masend(natl pid) {
	// controlli
	if(!esecuzione->maowner) {
		flog(LOG_WARN, "il processo non ha msg area");
		c_abort_p();
		return;
	}

	if(pid == esecuzione->id) {
		flog(LOG_WARN, "il processo non puo' inviare a se' stesso");
		c_abort_p();
		return;
	}
	...
	// altro proc
	des_proc* altro = proc_table[pid];

	if(!altro) {
		flog(LOG_DEBUG, "il processo desiderato non esiste");
		return;
	}

	if(altro->maowner) {
		flog(LOG_DEBUG, "il processo desiderato ha gia' una msg area");
		return;
	}

	if(altro->livello == LIV_SISTEMA) {
		flog(LOG_WARN, "il processo non puo' inviare a processi sistema");
		c_abort_p();
		return;
	}

	// prepariamoci prima le traduzioni
	paddr traduzioni[MSG_AREA_PAGES];
	int i = 0;

	// adesso si vuole:
	// 1) unmappare le tabelle della msg area di esecuzione
	unmap(esecuzione->cr3, ini_ma_p, fin_ma_p, 
			[&](vaddr v, paddr p, int) {
				traduzioni[i++] = p;
	...
	// 2) mappare le tabelle della msg area di esecuzione alla msg area di pid
	vaddr v = map(altro->cr3, ini_ma_p, fin_ma_p, BIT_RW | BIT_US,
			[&](vaddr v) { 
				return traduzioni[i++];
			});
	if (v != fin_ma_p) {
		// qui devi fare pulizia
		unmap(altro->cr3, ini_ma_p, v, [](vaddr, paddr, int) {});
		
		// riparti
		i = 0;

		map(esecuzione->cr3, ini_ma_p, fin_ma_p, BIT_RW | BIT_US,
			[&](vaddr v) { 
				return traduzioni[i++];
			});
