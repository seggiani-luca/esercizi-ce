...
extern "C" void c_masend(natl pid) {
	...
	// adesso si vuole:
	// 1) mappare le tabelle della msg area di esecuzione alla msg area di pid
	vaddr v = map(altro->cr3, ini_ma_p, fin_ma_p, BIT_RW | BIT_US,
			[](vaddr v) { return trasforma(esecuzione->cr3, v); });
	if (v != fin_ma_p) {
		unmap(altro->cr3, ini_ma_p, v, [](vaddr, paddr, int) {});
		return;
	}

	// 2) unmappare le tabelle della msg area di esecuzione
	unmap(esecuzione->cr3, ini_ma_p, fin_ma_p, [](vaddr v, paddr, int) { invalida_entrata_TLB(v); });
