...
/**
 * @brief Registra il processo come lettore dalla MQ.
 * @pre Il processo non deve essere già registrato.
 */
extern "C" void mq_reg();

/**
 * @brief Accoda un messaggio sulla MQ.
 * @pre Il processo non deve essere registrato come receiver.
 * @pre Non ci devono essere problemi di Cavallo di Troia.
 *
 * @note blocca il processo fino a quando il messaggio non è
 * stato consumato da tutti i receiver registrati fino al momento dell'invio.
 *
 * @param msg	puntatore al messaggio da accodare
 * @param len   lunghezza in byte del messaggio
 *
 * @return	false se non è stato possibile accodare il messaggio (MQ piena)
 */
extern "C" bool mq_send(char *msg, natq len);

/**
 * @brief Restituisce il prossimo messaggio accodato nella MQ e non ancora letto dal processo.
 * @note Se il buffer non è sufficiente a contenere il messaggio, consuma comunque il messaggio
 * ma non modifica il buffer di ricezione.
 *
 * @pre Il processo deve essere registrato come lettore.
 * @pre Non ci devono essere problemi di Cavallo di Troia.
 *
 * @param buf	puntatore al buffer di ricezione
 * @param size  dimensione in byte del buffer di ricezione
 *
 * @return 	la dimensione originale del messaggio (anche se maggiore della dimensione
 *              del buffer)
 */
extern "C" natq mq_recv(char *buf, natq size);
//   ESAME 2025-01-09 )
