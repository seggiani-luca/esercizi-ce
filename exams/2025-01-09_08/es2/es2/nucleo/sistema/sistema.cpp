...
// ( ESAME 2025-01-09
	...
	bool mq_reader;
	/// indice del prossimo messagio da leggere
	natq mq_ntr;
	/// @brief puntatore al buffer/messaggio se il processo è bloccato nella MQ, nullptr altrimenti
	char *mq_buf;
...
struct mq_msg {
	/// processo che sta inviando il messaggio
	des_proc *sender;
...
struct mq_des {
	...
	/// coda dei processi bloccati in lettura
	des_proc *w_readers;
	/// numero dei processi bloccati in lettura
	natq nwaiting;
};
...
/**
 * @brief Incremento circolare di un indice nella MQ.
 *
 * @param idx	indice da incrementare
 *
 * @return 	indice incrementato circolarmente
 */
 ...
 * @brief MQ piena?
 *
 * @return 	true se la MQ è piena, false altrimenti
 */
bool mq_full()
{
...
// [ ESAME 2025-01-09
/// @ingroup esame
/// @note	Inizializza i campi relativi alle MQ
...
// [ ESAME 2025-01-09
/// @ingroup esame
/// @note Rende la MQ consistente dopo la terminazione di un processo.
	if (p->mq_reader) {
		mq_des *mq = &message_queue;
...
/**
 * @brief Copia un messaggio dal processo corrente ad un altro
 *
 * @param receiver	processo che deve ricevere il messaggio
 */
...
/**
 * @brief Copia un messaggio al processo corrente da un altro 
 *
 * @param sender	processo che ha accodato il messaggio
 */
void mq_msgpull(des_proc *sender)
{
	if (esecuzione->mq_buflen < sender->mq_buflen)
		return;
...
/// Parte C++ della primitiva mq_reg()
extern "C" void c_mq_reg()
{
	flog(LOG_DEBUG, "mq_reg");
...
/// Parte C++ della primitiva mq_send()
extern "C" void c_mq_send(char *msg, natq len)
{
	flog(LOG_DEBUG, "mq_send, msg=%p, len=%lu", msg, len);
	
	mq_des *mq = &message_queue;

	// controlli
	if(esecuzione->mq_reader) { // è lettore
		flog(LOG_WARN, "mq_send: processo lettore");
		c_abort_p();
		return;
	}

	if(mq_full()) { // la coda è piena
		esecuzione->contesto[I_RAX] = false;
		return;
	}
	
	if (!c_access(int_cast<vaddr>(msg), len, false, false)) { // buf invalido
		flog(LOG_WARN, "mq_send: parametri non validi");
		c_abort_p();
		return;
	}
	
	flog(LOG_DEBUG, "mq_send, *msg=%.*s,", (int) len, msg);
...
/// Parte C++ della primitiva mq_recv()
extern "C" void c_mq_recv(char *buf, natq size)
{
	flog(LOG_DEBUG, "mq_recv, buf=%p, size=%lu", buf, size);

	mq_des *mq = &message_queue;

	// controlli
	if(!esecuzione->mq_reader) { // non è lettore
		flog(LOG_WARN, "mq_recv: processo non lettore");
		c_abort_p();
		return;
	}
	
	if (!c_access(int_cast<vaddr>(buf), size, true, false)) { // buf invalido
		flog(LOG_WARN, "mq_recv: parametri non validi");
		c_abort_p();
		return;
	}

	// prepara il descrittore
	esecuzione->mq_buf = buf;
	esecuzione->mq_buflen = size;

	// controlla se c'è un nuovo messaggio
	if(esecuzione->mq_ntr == mq->tail) { // non c'è nulla da leggere
		// mettiti in attesa
		...
		schedulatore();
		return;
	} else { // c'è qualcosa da leggere
		// il messaggio
		mq_msg* m = &mq->mq[esecuzione->mq_ntr];
		
		m->toread--;
		
		// il processo che lo ha accodato
		des_proc* p = m->sender;

		// intanto restituisci la dimensione
		natq len = p->mq_buflen;
		esecuzione->contesto[I_RAX] = len;

		// poi copia il mesaggio vero e proprio
		mq_msgpull(p);
