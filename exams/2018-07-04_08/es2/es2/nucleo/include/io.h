 ...
 *
 * @return quantità di byte liberi nello heap del modulo I/O
 */
...
/**
 * @brief		Riceve un messaggio dalla rete.	
 *
 * @param[out]	src	Indirizzo del computer che ha inviato il messaggio
 * @param	buf	Buffer destinato a contenere il messaggio
 * @param[in,out] len	in: lunghezza del buffer, out: lunghezza del messaggio
 *
 * @return		false se il buffer non è sufficiente a contenere il
 * 			messaggio, true altrimenti 
 */
extern "C" bool receive(natl& src, char *buf, natq& len);
//   ESAME 2018-07-04 )
