...
/// Dimensione della coda circolare delle periferiche CE.
static const natl DIM_RING = 8;

/**
 * @brief Prossimo indice nella coda circolare.
 *
 * @param pos	indice iniziale
 *
 * @return 	indice successivo
 */
natl ring_next(natl pos)
{
	return (pos + 1) % DIM_RING;
}

/**
 * @brief Coda circolare piena?
 *
 * @param tail	posizione di coda
 * @param head	posizione di testa
 *
 * @return true se la coda è piena, false altrimenti
 */
bool ring_full(natl tail, natl head)
{
	return ring_next(tail) == head;
}

/// Struttura di un messaggio
struct msg {
	/// @name Intestazione del messaggio
	/// @{
	natl src;	///< indirizzo sorgente
	natl dst;	///< indirizzo di destinazione
	natl len;	///< dimensione (escludendo l'intestazione)
	/// @}
	/// Messaggio vero e proprio
	char payload[MAX_PAYLOAD];
};

/// Descrittore di periferica di rete
struct des_net {
	/// @name Indirizzi dei registri
	/// @{
	ioaddr iHEAD,	///< posizione di testa
	       iTAIL,	///< posizione di coda
	       iRING;	///< indirizzo fisico della coda di descrittori
	/// @}
	/// Coda cicolare di descrittori
	slot s[DIM_RING];
	/// Indice del semaforo di mutua esclusione
	natl mutex;
	/// Indice del semaforo che conta i messaggi disponibili
	natl messages;
	/// Ultimo valore letto dal registro HEAD
	natl old_head;
	/// Indice del prossimo buffer da leggere
	natl toread;
};

/// Unica istanza del descrittore di periferica di rete.
struct des_net net;

// ( SOLUZIONE 2018-07-04
extern "C" bool c_receive(natl& src, char *payload, natq& len) {
	if(!access(&src, 1, true, false)) {
		abort_p();
	}
	if(!access(payload, len, true, false)) {
		abort_p();
	}
	if (!access(&len, 1, true, false)) {
		abort_p();
	}

	sem_wait(net.mutex); // mutua c_receive

	sem_wait(net.messages); // segnale nuovo messaggio

	// se sei qua è perche è arrivato un messaggio
	
	// bisogna leggere a toread ed incrementare
	slot this_s = net.s[net.toread];

	// prendi il messaggio
	msg* m = reinterpret_cast<msg*>(this_s.addr);

	// intanto puoi leggerlo?
	if(m->len > len) {
		sem_signal(net.mutex); // mutua c_receive
		return false;
	}
	...
	// copialo
	memcpy(payload, m->payload, len);

	// libera lo slot che hai usato
	outputl(net.toread, net.iTAIL);
...
/**
 * @brief 	Processo esterno associato all'interfaccia di rete.
 *
 * Per ogni nuovo messaggio, inserisce un gettone nel semaforo des_net::messages.
 *
 */
extern "C" void estern_net(natq i)
{
	for (;;) {
		natl new_head = inputl(net.iHEAD);
		for ( ; net.old_head != new_head; net.old_head = ring_next(net.old_head)) {
			sem_signal(net.messages);
		}
		wfi();
	}
}
...
/**
 * @brief Inizializza l'interfaccia di rete.
 *
 * Trova la periferica CE, ne inizializza il descrittore, alloca il la coda
 * circolare di descrittori di buffer e ne comunica l'indirizzo fisico
 * all'interfaccia, avvia un processo esterno.
 *
...
bool net_init()
{
	...
	if (!pci::find_dev(bus, dev, fun, 0xedce, 0x1234)) {
		flog(LOG_ERR, "net non trovato");
		return false;
	}
	...
	net.mutex = sem_ini(1);
	if (net.mutex == 0xFFFFFFFF) {
		flog(LOG_ERR, "semafori insufficienti");	
		return false;
	}
	net.messages = sem_ini(0);
	if (net.messages == 0xFFFFFFFF) {
		flog(LOG_ERR, "semafori insufficienti");
		return false;
	}
	natb irq = pci::read_confb(bus, dev, fun, 0x3c);
	activate_pe(estern_net, 0, MIN_EXT_PRIO + INTR_TIPO_NET, LIV_SISTEMA, irq);

	/* comunichiamo l'indirizzo del ring all'interfaccia */
	outputl(trasforma(net.s), net.iRING);
	/// @todo Inizializzare il ring
// ( SOLUZIONE 2018-07-04
	net.toread = net.old_head = 0;
	
	natl size = MAX_PAYLOAD + 3 * sizeof(natl);
	
	for(natl i = 0; i < DIM_RING; i++) {
		void* buf = new(std::align_val_t{4096}) char[size];
	...
	// outputl(0, net.iHEAD); // non potresti nemmeno scriverci
	// outputl(0, net.iTAIL);
//   SOLUZIONE 2018-07-04 )
	flog(LOG_INFO, "net: %02x:%02x.%1x base=%04x IRQ=%u", bus, dev, fun, base, irq);
	return true;
}
//   ESAME 2018-07-04 )
...
// [ ESAME 2018-07-04
	if (!net_init()) {
		flog(LOG_ERR, "inizializzazione net fallita");
		abort_p();
	}
//   ESAME 2018-07-04 ]
	*p_io_init_done = 1;
