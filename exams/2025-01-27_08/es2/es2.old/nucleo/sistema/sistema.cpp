...
// ( ESAME 2025-01-27
	...
	bool bloccato;
	/// è arrivata una richiesta di interruzione
	bool ricevuto_intr;
//   ESAME 2025-01-27 )
 ...
 *  @param evreg	se true, il processo verrà notificato ogni volta
 *  		        che un processo utente termina
 */
extern "C" void c_activate_pe(void f(natq), natq a, natl prio, natl liv, natb irq, bool evreg)
{
	des_proc*	p;			// des_proc per il nuovo processo
...
{
	if (esecuzione->notificato || esecuzione->ricevuto_intr) {
		if (!esecuzione->registrato)
...
extern "C" void c_evget()
{
	...
	// c'è un interruzione esterna, prima manda quella
	...
	// hai finito di inoltrare la notifica
	if (!risposte_mancanti) {
