...
struct vce_des {
	...
	natl n;
	/// indice di un semaforo di sincronizzazione (per l'inserimento in coda piena!)
	natl sync;
...
extern "C" bool c_vcenew()
{
	...
	// controlla non sia già allocata
	if(v != nullptr) {
...
//   SOLUZIONE 2025-01-27 )
	...
	// chi ha servito se lo ricorda lui
	natl current = 0;

	// qui da qualche parte dovrai usare la evget() per capire cosa sta succedendo
	while(true) {
		//	perchè ti svegliano?
		natl ev = evget();
		...
		if(ev == 0) { // interruzione esterna, il disp. ha finito
			ce.busy = false;

			// invia un altro byte
			
			// ottieni la prossima vce
			vce_des* next_vce = vce_des_next(current);
			if(next_vce != nullptr) {
				// leggi il prossimo byte
				char c = vce_des_read(next_vce);
				
				// invialo
				outputb(c, ce.iTBR);
				ce.busy = true;

				// per chi aspettava di inserire
				if(next_vce->waiting) {
					next_vce->waiting = false;
					sem_signal(next_vce->sync);
				} else if (!next_vce->n && next_vce->terminated) {
					delete next_vce;
					ce.vces[current] = nullptr;
				}
			}
		} else { // processo terminato, ev è l'id
			vce_des* &v = ce.vces[ev];

			if(v != nullptr) {
				v->terminated = true;

				// qui lo fa solo se è vuoto
				if(!v->n) {
					delete v;
					v = nullptr;
				}
