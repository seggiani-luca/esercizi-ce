 ...
 * @param evreg		se true, registra il processo per la ricezione degli eventi di terminazione
 * @return		id del nuovo processo, o 0xFFFFFFFF in caso di errore
 */
extern "C" natl activate_pe(void f(natq), int a, natl prio, natl liv, natb irq, bool evreg = false);

/**
 * @brief Attende la prossima richiesta di interruzione o evento.
 */
extern "C"  void wfi();
//   ESAME 2025-01-27 )
...
/**
 * @brief Alloca memoria che sia allineata in memoria fisica.
 *
 ...
 */
extern "C" void* phys_alloc(size_t size, std::align_val_t align);

/**
 * @brief Dealloca memoria allocata tramite @ref phys_alloc().
 *
 * @param ptr		puntatore alla memoria da liberare
 */
extern "C" void phys_dealloc(void *ptr);

// ( ESAME 2025-01-27


/**
 * @brief Ottiene la causa del ritorno dalla wfi()
 * @ingroup esame
 *
 * @pre il processo deve essere registrato per la ricezione delle notifiche
 *
 * @return 0xFFFFFFFF se non ci son variazioni rispetto all'ultima invocazione;
 *         0 in caso di nuova richiesta di interruzione; altrimenti il pid di
