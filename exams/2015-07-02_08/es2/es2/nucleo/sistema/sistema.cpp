...
extern "C" void c_shmem_attach(natl id) {
	// controlli
	if(!shmem_valid(id)) {
		flog(LOG_WARN, "shmem inesistente");
		c_abort_p();
	}
	...
	// prossimo indirizzo virtuale disponibile
	vaddr next_avail = esecuzione->avail_addr + s->npag * DIM_PAGINA;

	if(next_avail >= fin_utn_c || next_avail < esecuzione->avail_addr) {
		// finiti indirizzi virtuali
		esecuzione->contesto[I_RAX] = 0;
		return;
	}
	
	// attacca shmem 
	natl frame = s->first_frame_number;
	
	// usiamo vaddr map(paddr tab, vaddr begin, vaddr end, natl flags, T& getpaddr, int ps_lvl = 1)
	vaddr res = map(
			esecuzione->cr3, 
			esecuzione->avail_addr, 
			next_avail,
			BIT_RW | BIT_US,
			[&frame](vaddr v) {
				paddr p = frame * DIM_PAGINA;
				frame = vdf[frame].next_shmem; // prossimo frame
				return p;
			}
	);
	if(res != next_avail) { // ricordati di controllare se ha avuto successo!
		unmap(
				esecuzione->cr3,
				esecuzione->avail_addr,
				next_avail,
	...
	// ritorna
	esecuzione->contesto[I_RAX] = esecuzione->avail_addr;
	esecuzione->avail_addr = next_avail;
}
//   SOLUZIONE 2015-07-02 )
//   ESAME 2015-07-02 )
/// @}
