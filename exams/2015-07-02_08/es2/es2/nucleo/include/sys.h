...
/**
 * @brief Crea una nuova SHMEM.
 *
 * @note La primitiva si limita ad allocare i frame necessari e inserirli
 * 	in lista.
 *
 * @param npag	dimensione della SHMEM in pagine
 *
 * @return id della SHMEM, o 0xFFFFFFFF se non ci sono frame liberi a sufficienza
 */
extern "C" natl shmem_create(natq npag);

/**
 * @brief Aggiunge una SHMEM allo spazio di indirizzamento del processo corrente.
 *
 * @pre La SHMEM deve esistere.
 *
 * @param id	id della SHEM
 *
 * @return indirizzo virtuale della base della SHMEM (0 se non ci sono indirizzi virtuali
 * 	liberi sufficienti per mappare la SHMEM)
 */
