...
// [ ESAME 2024-07-17
/// @ingroup esame
/// @note Rispondiamo ad eventuali notifiche pendenti se il processo era registrato.
	if (esecuzione->registrato) {
		while (esecuzione->notificato)
			c_evget();
	}
/// @ingroup esame
/// @note Se necessario, notifichiamo la terminazione e rimandiamo la distruzione del processo.
...
/// processo la cui notifica è in corso
...
/**
 * @brief Parte C++ della primitiva evreg().
 */
...
{
	if(!esecuzione->registrato) return false;

	// bisogna anche rendere noto il ritorno, la wfi() non lo fa
	natq r = 0;

	if(esecuzione->notificato) r = 2;
...
/**
 * @brief Notifica la terminazione di un processo a tutti i processi registrati.
 *
 * @param src	processo terminato
 *
 * @return 	false se l'evento non deve essere notificato, true altrimenti
 */
...
/**
 * @brief Parte C++ della primitiva evget().
 */
extern "C" void c_evget()
{
	...
	if(!esecuzione->registrato) {
		// errore: non registrato
		return;
	}
	
	if(!esecuzione->notificato) {
		// errore: nulla da notificare
		return;
	}
	...
	if(risposte_mancanti == 0) {
		distruggi_processo(in_notifica);
		processi--;
		in_notifica = nullptr;
	
		if(terminati != nullptr) {
			des_proc* prossimo = rimozione_lista(terminati);
			inspronti();
			notify_event(prossimo);
			schedulatore();
		}
	}
}
...
/**
 * @brief Chiamata dagli handler per segnalare una nuova richiesta di interruzione.
 *
 * @param irq	irq da cui è arrivata la richiesta
 */
