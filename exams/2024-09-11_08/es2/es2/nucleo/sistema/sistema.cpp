...
/// @brief Tipo di lock posseduto da un processo su un rwlock
enum rw_states { 
	RW_NONE,	///< nessun lock
	RW_WRITER,	///< lock in scrittura
	RW_READER,	///< lock in lettura
	RW_UPGRADED	///< lock in lettura trasfromato in scrittura
};

struct des_rw;
/// @brief Descrittore di lock posseduto da un processo
struct des_proc_rw {
...
// ( ESAME 2024-09-11
	...
	/// descrittori dei lock posseduti
...
// [ ESAME 2024-09-11
/// @ingroup esame
/// @note All'inizio il processo non possiede lock.
...
/**
 * @brief Alloca un nuovo descrittore di rwlock.
 *
...
/**
 * @brief Controlla la validità di un id di rwlock.
 *
 * @param rw id da controllare
 *
 * @return  true se l'id corrisponde ad un rwlock allocato, false altrimenti
 */
...
/// Parte C++ della primitiva rw_init()
extern "C" natl c_rw_init()
{
	flog(LOG_DEBUG, "rw_init");
...
/**
 * @brief Trova il des_proc_rw associato ad un dato rwlock per un dato processo
 *
 * @note se r è nullptr si può usare per trovare un descrittore libero
 *
 * @param r	descrittore di rwlock o nullptr
 * @param p	processo 
 *
 * @return puntatore al descrittore, o nullptr se non trovato
 */
...
/// Parte C++ della primitiva rw_writelock()
extern "C" void c_rw_writelock(natl rw)
{
	flog(LOG_DEBUG, "rw_writelock, rw=%d", rw);
	...
	rp = rw_proc_find();
	if (!rp) { // esecuzione non ha des_proc_rw liberi
		esecuzione->contesto[I_RAX] = false;
		return;
	}
	rp->r = r;
...
/// Parte C++ della primitiva rw_readlock()
extern "C" void c_rw_readlock(natl rw)
{
	flog(LOG_DEBUG, "rw_readlock, rw=%d", rw);
...
void rw_wakeup_writers(des_rw *r) {
	des_proc* next_writer = rimozione_lista(r->w_writers);

	if(!next_writer) return;

	inserimento_lista(pronti, next_writer);

	r->writer = next_writer->id;
	
	des_proc_rw *nrp = rw_proc_find(r, next_writer);
	nrp->state = nrp->state == RW_READER ? RW_UPGRADED : RW_WRITER;
}


void rw_wakeup_readers(des_rw *r) {
	while(r->w_readers) {
		des_proc* next_reader = rimozione_lista(r->w_readers);

		inserimento_lista(pronti, next_reader);

		r->nreaders++;

		des_proc_rw *nrp = rw_proc_find(r, next_reader);
		nrp->state = RW_READER;
	}
}

extern "C" void c_rw_upgrade(natl rw) {
	flog(LOG_DEBUG, "rw_upgrade, rw=%d", rw);
	...
	if(!rp) {
		flog(LOG_WARN, "rw_upgrade(%d): nessun rwlock detenuto", rw);
		c_abort_p();
		return;
	}

	if (rp->state != RW_READER) {
		flog(LOG_WARN, "rw_upgrade(%d): nessun rwlock in lettura detenuto", rw);
		c_abort_p();
		return;
	}
...
extern "C" void c_rw_downgrade(natl rw) {
	flog(LOG_DEBUG, "rw_downgrade, rw=%d", rw);
	...
	if(!rp) {
		flog(LOG_WARN, "rw_downgrade(%d): nessun rwlock detenuto", rw);
		c_abort_p();
		return;
	}
