...
struct des_proc {
	...
	// parte pipe
	// array di pipe locali
	natl proc_pipes[MAX_OPEN_PIPES];
};
...
des_proc* crea_processo(void f(natq), natq a, int prio, char liv)
{
	...
	// inizializza pipe locali
	for(natl i = 0; i < MAX_OPEN_PIPES; i++) {
		p->proc_pipes[i] = 0xffffffff;
	}
...
// descrittori pipe
struct des_pipe {
	natl writer = 0xffffffff; // chi scrive
	natl reader = 0xffffffff; // chi legge
	natl w_pending = 0; // il numero di byte da scrivere
	const char *w_buf; // puntatore al buffer di scrittura
	natl r_pending = 0; // il numero di byte da leggere
	char *r_buf; // puntatore al buffer di lettura
};

des_pipe sys_pipes[MAX_PIPES];

// primitive pipe parte c++
extern "C" void c_openpipe(natl pipeid, bool writer) { // restituisce natl
	flog(LOG_DEBUG, "openpipe(), pipeid=%u, writer=%d", pipeid, writer);

	// controlli
	if(pipeid >= MAX_PIPES) { // pipeid invalido
		flog(LOG_WARN, "openpipe() ha abortito: pipeid invalido");
		c_abort_p();
		return;
	}

	des_pipe* pipe = &sys_pipes[pipeid]; // puntatore al pipe
	
	// ottieni ruolo proprio e dell'altro processo
	natl* this_role, other_role;
	if(writer) { // scrittore
		this_role = &pipe->writer;
		other_role = pipe->reader;
	} else { // lettore
		this_role = &pipe->reader;
		other_role = pipe->writer;
	}

	// controlla che il ruolo sia libero
	if(*this_role != 0xffffffff) {
		flog(LOG_WARN, "openpipe() ha interrotto: ruolo occupato");
		esecuzione->contesto[I_RAX] = 0xffffffff;
		return;
	}
	...
	for(natl i = 0; i < MAX_OPEN_PIPES; i++) {
		if(esecuzione->proc_pipes[i] == 0xffffffff) {
			free_slot = i;
		}
	}
	...
	// (solo ora) impone il ruolo 
	*this_role = esecuzione->id;
	
	// popola e fornisci al processo
	esecuzione->proc_pipes[free_slot] = pipeid;
	esecuzione->contesto[I_RAX] = free_slot;

	// schedula: se non hai liberato nessuno metti il corrente in attesa,
	// se hai liberato qualcuno metti questo o il corrente in esecuzione
	// nota successiva: pare l'esame si aspetti che chi ha chiamato la primitiva
	// viene sempre rimesso in esecuzione per primo
	schedulatore();
}

extern "C" void c_writepipe(natl slotid, const char *buf, natl n) { // restituisce bool
	...
	natl pipeid = esecuzione->proc_pipes[slotid];
	des_pipe* pipe = &sys_pipes[pipeid]; // puntatore al pipe
	
	if(pipe->writer != esecuzione->id) { // non è scrittore
	...
	// ora puoi effettuare la scrittura, in due passaggi:
	// 1) copia tutto quello che puoi a un eventuale buffer di lettura
	while(n > 0 && pipe->r_pending > 0) {
		*(pipe->r_buf) = *buf;
		pipe->r_buf++;
	...
	// 2) se resta qualcosa, inizializza il buffer di scrittura 
	// (sarà problema di un lettore leggerci)
	if(n) { // è rimasto qualcosa
		pipe->w_buf = buf;
		pipe->w_pending = n;
	} else { // non c'è altro
		esecuzione->contesto[I_RAX] = true;
	...
	// schedula: se hai liberato un lettore va in esecuzione lui, 
	// senno va in esecuzione un altro potenziale lettore,
	// altrimenti se hai concluso hai reinserito il corrente
	schedulatore();
}

extern "C" void c_readpipe(natl slotid, char* buf, natl n) { // restituisce bool
	...
	if(pipe->reader != esecuzione->id) { // non è lettore
	...
	// ora puoi effettuare la lettura, in due passaggi:
	// 1) leggi tutto quello che puoi da un eventuale buffer di scrittura 
	while(n > 0 && pipe->w_pending > 0) {
		*buf = *pipe->w_buf;
		pipe->w_buf++;
	...
	// 2) se resta qualcosa, inizializza il buffer di lettura 
	// (sarà problema di uno scrittore)
	if(n) { // è rimasto qualcosa
		pipe->r_buf = buf;
		pipe->r_pending = n;
	} else { // non c'è altro
...
extern "C" void c_closepipe(natl slotid) {
	flog(LOG_DEBUG, "closepipe(), slotid=%u", slotid);
	...
	inspronti();
	closepipe_extrem(slotid, esecuzione);
...
extern "C" void closepipe_extrem(natl slotid, des_proc* proc) {
	...
	// puoi ricavarti un id pipe valido
	natl pipeid = proc->proc_pipes[slotid];
	des_pipe* pipe = &sys_pipes[pipeid]; // puntatore al pipe
