...
// [ ESAME 2025-02-25
	/// @ingroup esame
	/// @brief identificatori delle pipe aperte dal processo
...
struct des_pipe {
	...
	/// processo lettore
	natl reader;
	/// numero di byte ancora da scrivere
	natl w_pending;
	/// se w_pending è maggiore di zero, puntatore ai byte ancora da scrivere
	const char *w_buf;
	/// numero di byte ancora da leggere
	natl r_pending;
	/// r_pending è maggiore di zero, puntatore al buffer destinato a contenere i byte ancora da leggere
	char *r_buf;
};
...
//   SOLUZIONE 2025-02-12 )
	...
	if (n) { // qui e' rimasto qualcosa 
		dp->w_pending = n; // lo metti nel buffer di scrittura e cazzi del prossimo
		dp->w_buf = buf;
	} else { // qui non e' rimasto nulla 
		dp->w_buf = nullptr; // liberi il buf di scrittura (perchè doveva essere pieno?)
		esecuzione->contesto[I_RAX] = true; // hai fatto
		inspronti(); // ti prepari a schedulare il corrente
	}
	
	if (!dp->r_pending && dp->r_buf) { // qui hai esaurito la lettura corrente
		dp->r_buf = nullptr; // liberi il buf di lettura
		des_proc *reader = proc_table[dp->reader];
		reader->contesto[I_RAX] = true;
		inserimento_lista(pronti, reader); // reinserisci il lettore
	}

	schedulatore(); // fai una preghiera e rimetti in esecuzione il prossimo
}
